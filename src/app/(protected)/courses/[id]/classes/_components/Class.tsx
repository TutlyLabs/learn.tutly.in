...
interface Attachment {
  ...
  title: string;
  link?: string | null;
  attachmentType: "ASSIGNMENT" | "GITHUB" | "ZOOM" | "OTHERS";
  dueDate?: Date;
}
...
export default function Class({
  ...
  const { mutate: toggleBookmarkMutation } = api.bookmarks.toggle.useMutation({
    onSuccess: () => {
      toast({
  ...
  const { mutate: deleteAttachment } = api.attachments.delete.useMutation({
    onSuccess: () => {
      toast({
  ...
  const handleDelete = () => {
    if (selectedAttachment) {
      void deleteAttachment({ id: selectedAttachment.id });
    }
  };
  ...
  const toggleBookmark = () => {
    void toggleBookmarkMutation({
      objectId: classId,
      category: "CLASS",
      causedObjects: { classId, courseId },
    });
  };
  ...
  useEffect(() => {
    ...
    void saveNotes();
  }, [debouncedNotes, classId, courseId, tags]);
